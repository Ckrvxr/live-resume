...
test('static resume stays read-only and supports export and browser printing', async () => {
  ...
  assert.notEqual(await page.locator('#print-button').evaluate(node => getComputedStyle(node).transitionDuration), '0s');
  ...
  assert.equal(await page.locator('#lock-button').count(), 0);
  assert.equal(await page.locator('.name').evaluate(node => node.isContentEditable), false);
  const markdownDownload = page.waitForEvent('download');
  await page.click('#markdown-button');
  const markdown = await readFile(await (await markdownDownload).path(), 'utf8');
  assert(markdown.includes('# 曹锐旋'));
  assert(markdown.includes('> 意向：嵌入式软件实习'));
  assert(markdown.includes('## 个人概况'));
  assert(markdown.includes('- 手机：+86 131 1332 8451'));
  ...
  for (const width of [320, 390, 760]) {
    ...
    assert.equal(await page.locator('#markdown-button').isVisible(), false);
    assert.equal(await page.locator('.page').evaluate(node => getComputedStyle(node).marginTop), '0px');
    assert.equal(await page.locator('.entry-head').first().evaluate(node => getComputedStyle(node).flexDirection), 'column');
    assert(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth));
    assert(await page.locator('.entry-title').evaluateAll(nodes => nodes.every(node => node.scrollWidth <= node.clientWidth + 1)));
  }
  for (const width of [768, 1024, 1440]) {
    await page.setViewportSize({ width, height: 1000 });
    assert.equal(await page.locator('#print-button').isVisible(), true);
    assert.equal(await page.locator('#markdown-button').isVisible(), true);
  }
